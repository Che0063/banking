...
def rule_suggestions():
    ...
    existing_rules = conn.execute("SELECT pattern,category,use_regex,enabled FROM rules WHERE enabled=1").fetchall()
    conn.close()
    mc = {}
    for r in rows:
    ...
    for merchant, counts in mc.items():
        ...
        if total >= 2 and top_count / total >= 0.8:
            ...
            if len(keyword) >= 4:
                # Check if an existing rule already covers this merchant → same category
                already_covered = False
                for rule in existing_rules:
                    try:
                        if rule["use_regex"]:
                            matched = bool(re.search(rule["pattern"], merchant, re.IGNORECASE))
                        else:
                            matched = rule["pattern"].lower() in merchant.lower()
                        if matched and rule["category"] == top_cat:
